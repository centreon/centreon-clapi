...
class CentreonHost {
	...
	public function hostExists($name) {
		 ...
		 * Get informations
		 */
		$DBRESULT =& $this->DB->query("SELECT host_name, host_id FROM host WHERE host_name = '".$this->encodeInHTML($name)."' AND host_register = '".$this->register."'");
	...
	public function _hostTemplateExists($name) {
		 ...
		 * Get informations
		 */
		$DBRESULT =& $this->DB->query("SELECT host_name, host_id FROM host WHERE host_name = '".$this->encodeInHTML($name)."' AND host_register = '0'");
		if ($DBRESULT->numRows() >= 1) {
			$host =& $DBRESULT->fetchRow();
			$DBRESULT->free();
			return $host["host_id"];
	...
	protected function getPollerID($name) {
		$request = "SELECT id FROM nagios_server WHERE name LIKE '".trim($this->encodeInHTML($name))."'";
		$DBRESULT =& $this->DB->query($request);
		if ($DBRESULT->numRows()) {
			$info =& $DBRESULT->fetchRow();
			$DBRESULT->free();
	...
	protected function addHost($information) {
		if (!isset($information["host_name"]) || !isset($information["host_address"]) || !isset($information["host_poller"])) {
			...
			$request = 	"INSERT INTO host (host_name, host_alias, host_address, host_register, host_activate, host_active_checks_enabled, host_passive_checks_enabled, host_checks_enabled, host_obsess_over_host, host_check_freshness, host_event_handler_enabled, host_flap_detection_enabled, host_process_perf_data, host_retain_status_information, host_retain_nonstatus_information, host_notifications_enabled) " .
						"VALUES ('".$this->encodeInHTML(trim($this->encode($information["host_name"])))."', '".$this->encodeInHTML(trim($this->encode($information["host_alias"])))."', '".$this->encodeInHTML(trim($information["host_address"]))."', '".$this->register."', '1', '2', '2', '2', '2', '2', '2', '2', '2', '2', '2', '2')";
			$this->DB->query($request);

			/***
			 * Get host ID.
			 */
			$host_id = $this->getHostID($this->encodeInHTML($information["host_name"]));
	...
	public function del($options) {
		 ...
		 * Get Host
		 */
		$request = "SELECT host_id FROM host WHERE host_name = '".$this->encodeInHTML($this->decode($options))."' LIMIT 1";
		$DBRESULT = $this->DB->query($request);
	...
	public function show($host_name = NULL) {
		$search = "";
		if (isset($host_name)) {
			$search = " AND (host_name like '%".$this->encodeInHTML($host_name)."%' OR host_alias LIKE '%".$this->encodeInHTML($host_name)."%') ";
		}
	...
	protected function setParameterHost($host_name, $parameter, $value) {
		...
		if ($parameter == "tpcheck" || $parameter == "notifcheck") {
			$request = "SELECT tp_id FROM timeperiod WHERE tp_name LIKE '".$this->encodeInHTML($value)."'";
			$DBRESULT =& $this->DB->query($request);
			$data = $DBRESULT->fetchRow();
			$value = $data["tp_id"];
		}
		 ...
		 */
		if ($parameter == "poller") {
			$host_id = $this->getHostID($this->encodeInHTML($host_name));
			$this->unsetPoller($host_id);
			$request = "SELECT id FROM nagios_server WHERE name LIKE '".$this->encodeInHTML($value)."'";
			$DBRESULT =& $this->DB->query($request);
			$data = $DBRESULT->fetchRow();
		...
		if ($parameter == "check_command") {
			$request = "SELECT command_id FROM command WHERE command_name LIKE '".$this->encodeInHTML($value)."'";
			$DBRESULT =& $this->DB->query($request);
			$data = $DBRESULT->fetchRow();
	...
	protected function setMacroHost($host_name, $macro_name, $macro_value) {
		...
		$host_id = $this->getHostID($this->encodeInHTML($host_name));
		$request = "SELECT COUNT(*) FROM on_demand_macro_host WHERE host_host_id = '".$this->encodeInHTML($host_id)."' AND host_macro_name LIKE '\$_HOST".$this->encodeInHTML($macro_name)."\$'";
		$DBRESULT =& $this->DB->query($request);
		$data =& $DBRESULT->fetchRow();
		if ($data["COUNT(*)"]) {
			$request = "UPDATE on_demand_macro_host SET host_macro_value = '".$this->encodeInHTML($macro_value)."' WHERE host_host_id = '".$this->encodeInHTML($host_id)."' AND host_macro_name LIKE '\$_HOST".$this->encodeInHTML($macro_name)."\$' LIMIT 1";
			$DBRESULT =& $this->DB->query($request);
			return 0;
		} else {
			$request = "INSERT INTO on_demand_macro_host (host_host_id, host_macro_value, host_macro_name) VALUES ('".$this->encodeInHTML($host_id)."', '".$this->encodeInHTML($macro_value)."', '\$_HOST".$this->encodeInHTML($macro_name)."\$')";
			$DBRESULT =& $this->DB->query($request);
			return 0;
		}
	}
	...
	protected function delMacroHost($host_name, $macro_name) {
		...
		$host_id = $this->getHostID($this->encodeInHTML($host_name));
		$request = "DELETE FROM on_demand_macro_host WHERE host_host_id = '".$this->encodeInHTML($host_id)."' AND host_macro_name LIKE '\$_HOST".$this->encodeInHTML($macro_name)."\$'";
		$DBRESULT =& $this->DB->query($request);
		return 0;
	}
	...
	public function enable($options) {
		...
		$host_id = $this->getHostID($this->encodeInHTML($options));
